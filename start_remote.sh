...
sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no $USER@$HOST "bash -s" << EOF
    ...
    nohup \$PYTHON_CMD odoo18/odoo/odoo-bin -c odoo18/config/odoo.conf --db_host=localhost --db_password=prompttech --http-port=\$PORT --longpolling-port=\$((PORT+1)) --workers=0 > odoo_remote.log 2>&1 &

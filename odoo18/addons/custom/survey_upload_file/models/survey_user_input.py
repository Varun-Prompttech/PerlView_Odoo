...
class SurveyUserInput(models.Model):
    ...
    def _save_line_simple_answers(self, question, old_answers, answer):
        ...
        file_ids = vals.get('value_file_data_ids', [])

        # Default to skipped=True, change to False if at least one valid file name exists
        vals['skipped'] = True
        if file_ids:
            attachments = self.env['ir.attachment'].browse(file_ids)
            for attachment in attachments:
                if attachment.name:  # or use attachment.display_name if needed
                    vals['skipped'] = False
                    break

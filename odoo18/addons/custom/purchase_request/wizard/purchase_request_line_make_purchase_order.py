...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        request_lines = self.env["purchase.request.line"].browse(request_line_ids).filtered(
            lambda l: l.purchase_state not in ('draft','purchase', 'done')
        )

        if not request_lines:
            return res

        res["item_ids"] = self.get_items(request_lines.ids)
